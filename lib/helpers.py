...
class ExcelHelpers:
    ...
    @ classmethod
    def get_col_by_name(cls, sheet: worksheet, name: str, header_row: int | None = 1) -> int:
        """
        Search row for a cell with a particular value and return it's column number
        Return value is 1-based
        """
        for col in sheet.iter_cols():
            for ind in range(1, 3):  # checks previous row as well in case of merged cells
...
class BaseWorkBookData(BaseWorkBook):
    ...
    def as_filtered_list(self, fields: Iterable, values: Iterable):
        ...
        def _check_fields(record, fields, values):
            ...
            return True
        return list(
            filter(lambda record: _check_fields(record, fields, values),
                   self.records)
        )
    ...
    def __init__(self,
        ...
        try:
            ...
            for row in sheet.iter_rows(min_row=header_row+1, max_col=max_col, values_only=True):
    ...
    def __call__(cls, arg: str, *args, **kwargs):
        ...
        if key not in cls._instances:
            cls._instances[key] = super(
                SingletonWithArg, cls).__call__(arg, *args, **kwargs)
