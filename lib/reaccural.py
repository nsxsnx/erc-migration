...
class Reaccural:
    ...
    def try_decompose_to_zero(self) -> None:
        ...
        for _ in range(MAX_DEPTH):
            try:
                prev_accural = Decimal(self.account_data.get_service_month_accural(
                    date := date.previous,
                    'Тепловая энергия для подогрева воды')).quantize(Decimal('0.01'))
            except NoServiceRow:
                continue
        ...
        for _ in range(MAX_DEPTH):
            try:
                prev_accural = Decimal(self.account_data.get_service_month_accural(
                    date := date.previous,
                    'Тепловая энергия для подогрева воды')).quantize(Decimal('0.01'))
                second_prev_accural = Decimal(self.account_data.get_service_month_accural(
                    date.previous,
                    'Тепловая энергия для подогрева воды')).quantize(Decimal('0.01'))
            except NoServiceRow:
                continue
            rec.append(ReaccuralMonthRec(date, float(prev_accural)))
            floating_sum += prev_accural
            if floating_sum < second_prev_accural:
                rec.append(ReaccuralMonthRec(
                    date.previous, float(floating_sum)))
                self.records = rec[::-1]
                self.valid = True
                return
    ...
    def __init__(self,
        ...
        if self.valid:
            return
        try:
            _ = Decimal(self.account_data.get_service_month_accural(
                self.date.next, 'Тепловая энергия для подогрева воды'))
        except NoServiceRow:
            # account is closed, try second algorithm
            self.try_decompose_to_previous_accurance()
        if not self.valid:
            self.records.append(ReaccuralMonthRec(
                reaccural_date.previous, reaccural_sum))
